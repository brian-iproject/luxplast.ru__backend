	...
	<? if ($arResult["SUCCESS"] != "Y") { ?>
		<? if ($arResult["VIRTUAL_FIELDS"]) { ?>
          ...
          <? if ($arParams["FORM_NAME"]) { ?>
						<div class="h3"><?=$arParams["FORM_NAME"]?></div>
          <? } ?>
          ...
          <? } ?>
	            ...
	            $strFieldPlaceholder = ' placeholder="'.$arFieldParams["PARAMS"]["PLACEHOLDER"].'"';
						...
						$strFieldClass = ' class="site-form__field-'.ToLower($arFieldParams["PARAMS"]["TYPE"]).' js-moving-placeholder';
						if ($arFieldParams["ERROR"]) {
	              $strFieldClass = $strFieldClass . ' site-form__field-'.ToLower($arFieldParams["PARAMS"]["TYPE"]).'--error"';
	          } else {
	              $strFieldClass = $strFieldClass . '"';
						...
						?>
					<div class="site-form__item site-form__item--<?=ToLower(str_replace($arParams['FIELDS_PREFIX'].'_', '', $fieldname))?><? if ($arFieldParams["ERROR"]) { ?> site-form__item--error<? } ?>">
	            ...
	            <input type="email" name="<?=$fieldname?>" value="<?=$arFieldParams["VALUE"]?>"<?=$strField?>>
						...
						<? if ($arFieldParams["ERROR"]) { ?>
						 <span class="site-form__error">Поле «<?=$arFieldParams['NAME']?>» должно быть заполнено</span>
						<? } ?>
	        </div>
					<? } ?>
                ...
                <? if ($arResult["CAPTCHA"]["ERROR"] == "C") { ?><br><span class="error_list">Неверно введены символы</span><? } ?>
					...
					<div class="site-form__item site-form__item--privacy-policy">
						<input type="checkbox" required checked class="site-form__field-checkbox"> <span>Даю согласие на обработку <a href="/privacy-policy/" target="_blank">персональных данных</a>.</span>
					</div>
			    <div class="site-form__item site-form__item--button">
			        <input type="hidden" name="<?=$arResult["SEND_BUTTON_NAME"]?>" value="Y">
				      <button class="site-button site-button--fill site-form__field-button"><span class="site-button__text">Отправить</span></button>
			    </div>
					<div class="site-form__item site-form__item--required-text">
						<span class="star-required">*</span> &mdash; обязательные поля
					</div>
